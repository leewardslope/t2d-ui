...
# This script is used to configure the essentials of t2d and install install Forem(for now)
...
## 6 => Downloading the latest whip-dokku.sh or easy-dokku.sh and placing in users VPS.
...
function funIntro() #Added Color
{
    ...
    whiptail --title "A small Notification" --msgbox "Presently this script offers Two Important Functions.\n
    1. Automatic (Step by Step Installation)
    ...
    2. Assuring that you know all prerequisites will make it easy to install.\n" 20 70
    wait
    '
...
function funUpdatemenu()
{
    ...
    then
        ...
        '
        APP=$(whiptail --title "Enter the Name of your App" --inputbox "Here is a list of your Existing Apps:\n$(cat -n test.tmp)" 15 60 3>&1 1>&2 2>&3)
        exitstatus=$?
        if [ $exitstatus = 0 ]; then
            echo "Your Chosen App:" $APP
            for OUTPUT in $(cat test.tmp)
            do
                if [[ "$OUTPUT" == "$APP" ]];
                then
            ...
            whiptail --title "Error" --msgbox "You might have entered a wrong APP name; Choose Ok to re-enter/continue." 10 60
            funUpdatemenu
        else
...
function funForem()
{
    ...
    whiptail --title "Pre-Requisites" --msgbox "In order for you to have a successful Forem Installation.\n
    ...
    wait
    whiptail --title "Mush Have ENV Variables" --msgbox "Please, do not continue without these ENV variables:\n
        APP_DOMAIN
        APP_PROTOCOL
        COMMUNITY_NAME
        FOREM_OWNER_SECRET
